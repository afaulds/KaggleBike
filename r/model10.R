...
Train <- function(data)
{
	...
	x$count <- NULL
	x$casual <- NULL
	model1 <- gbm(registered ~ ., data = x, interaction.depth = 10, n.trees = 1000, distribution = "gaussian")

	x <- data
	x$datetime <- NULL
	x$count <- NULL
	x$registered <- NULL
	model2 <- gbm(casual ~ ., data = x, interaction.depth = 10, n.trees = 1000, distribution = "gaussian")
...
{
	outcome1 <- predict(object$model1, data, n.trees = object$model1$n.trees)
	outcome2 <- predict(object$model2, data, n.trees = object$model2$n.trees)
